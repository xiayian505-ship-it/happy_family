(function () {
  ...
  function renderCalendar() {
    const year=view.getFullYear(), month=view.getMonth()+1;
    $("calendar-title").textContent=`${year} 年 ${month} 月`;
    const stats=C.cycleStats(data);
    const daysInMonth=new Date(year,month,0).getDate();
    const monthData={year,month,firstWeekday:new Date(year,month-1,1).getDay(),days:Array.from({length:daysInMonth},(_,i)=>i+1)};
    $("calendar").replaceChildren();
    CalendarMonthGridView.render($("calendar"),monthData,{
      titleFormatter:()=>"",
      renderDay(cell,day,context){
        const date=`${context.year}-${pad(context.month)}-${pad(day)}`;
        const button=document.createElement("button");button.type="button";button.className="day";button.dataset.date=date;
        button.textContent=String(day);const tags=[];
        if(date===C.todayISO()){button.classList.add("today");tags.push("今天");}
        if(date===selected)button.classList.add("selected");
        if(actual(date)){button.classList.add("period");tags.push("月經");}
        else if(predicted(date,stats)){button.classList.add("forecast");tags.push("預估月經");}
        else if(stats.fertileStart&&date>=stats.fertileStart&&date<=stats.fertileEnd){button.classList.add("fertile");tags.push("粗估易孕");}
        if(data.daily[date])button.classList.add("has-note");
        button.setAttribute("aria-label",`${date}${tags.length?"，"+tags.join("，"):""}`);
        if(tags.length){const marker=document.createElement("span");marker.className="marker";marker.textContent=tags[0];button.append(marker);}
        cell.append(button);
      }
    });
  }
  ...
  $("prev-month").onclick=()=>{const next=CalendarMonthSequence.shift(view.getFullYear(),view.getMonth()+1,-1);view=new Date(next.year,next.month-1,1);renderCalendar();};$("next-month").onclick=()=>{const next=CalendarMonthSequence.shift(view.getFullYear(),view.getMonth()+1,1);view=new Date(next.year,next.month-1,1);renderCalendar();};$("today-button").onclick=()=>{view=new Date();selected=C.todayISO();renderCalendar();};
  ...
  $("export-button").onclick=()=>{DataBackup.downloadJson(`slowly-cycle-${C.todayISO()}.json`,data);toast("備份已下載");};
  $("import-button").onclick=async()=>{
    const file=$("import-file").files[0];if(!file){alert("請先選擇 JSON 檔案。");return;}
    const previous=data;
    const result=await SafeImport.run({
      prepare:async()=>C.parseBackup(JSON.stringify(await DataBackup.readJsonFile(file))),
      validate:next=>C.validate(next).ok,
      commit:async next=>{
        if(!confirm(`已驗證備份：${next.periods.length} 筆月經紀錄、${Object.keys(next.daily).length} 筆身體紀錄。\n\n確定完全覆蓋目前資料？此動作無法復原。`))return {cancelled:true};
        await persist(next,"備份已還原");$("backup-dialog").close();return {cancelled:false};
      },
      rollback:async()=>{data=previous;render();}
    });
    if(!result.ok)alert("匯入失敗，目前資料未變更。\n"+(result.error?.message||"無法完成還原。"));
  };
